...
func (v *Validator) getErrorMessages(errs []string) (messages []string) {
  if len(errs) == 0 {
    return []string{}
  }
  ...
  if msg, ok := v.messages[errs[0]]; ok {
    messages = append(messages, fmt.Sprintf(msg, opts...))
  }
...
func (v *Validator) applyRule(rawRule string, value interface{}) []string {
  ...
  return v.getErrorMessages(errs)
}
...
func (v *Validator) Validate(input interface{}) (map[string][]string, error) {
  ...
  for i := 0; i < value.NumField(); i++ {
    ...
    for _, rawRule := range fieldRules {
      if errs := v.applyRule(rawRule, fieldValue); len(errs) > 0 {
